...
class Lydia(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        lusid_api_factory= lusid.utilities.ApiClientFactory(token=RefreshingToken(),api_secrets_filename="secrets.json")
        api_client = lusid_api_factory.api_client
        identity_api_client = identity_api.build(api_secrets_filename="secrets.json", token=api_client.configuration.access_token)
